...
function preprocess28(canvasEl) {
  ...
  const pad = Math.max(6, Math.round(Math.max(bw, bh) * 0.15));
  ...
  tmpCtx.drawImage(canvasEl, minX, minY, cropW, cropH, offX * 28 / size, offY * 28 / size, cropW * 28 / size, cropH * 28 / size);
  const tmpData = tmpCtx.getImageData(0, 0, 28, 28);
  const floatArray = new Float32Array(28 * 28);
  for (let i = 0; i < 28 * 28; i++) {
    floatArray[i] = (0.299 * tmpData.data[i * 4] + 0.587 * tmpData.data[i * 4 + 1] + 0.114 * tmpData.data[i * 4 + 2]) / 255.0;
  }
  return floatArray;
}
